...
<div class="flex flex-col">
    <div class="flex justify-center">
        ...
        <div class="border-b border-gray-200">
            ...
            <a @click="authPanel(tab.name)" v-for="tab in tabs" :key="tab.name" :class="[tab.current ? 'border-green-400 text-green-600' : 'border-transparent text-gray-500 hover:text-green-700 hover:border-green-300', 'group inline-flex items-center py-4 px-1 border-b-4 rounded font-medium text-sm']" :aria-current="tab.current ? 'page' : undefined">
                <component :is="tab.icon" :class="[tab.current ? 'text-green-500' : 'text-gray-400 group-hover:text-green-400', '-ml-0.5 mr-2 h-8 w-5']" aria-hidden="true" />
                <span>{{ tab.name }}</span>
            </a>
            </nav>
        </div>
